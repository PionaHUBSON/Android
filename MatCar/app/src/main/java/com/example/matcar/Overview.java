...
public class Overview extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener {
    ...
    static Button choose_mechanic_overview;
    private String date = "";
    ...
    public static Dialog myDialog_overview;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        if (getSupportActionBar() != null) {
            getSupportActionBar().setTitle("Ogólny przegląd");
        }
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_overview);

        choose_mechanic_overview = findViewById(R.id.overview_choose_mechanic_button);
        //POP_UP_WINDOW
        myDialog_overview = new Dialog(this);
        //POP_UP_WINDOW
        //FIREBASE
        ...
        Menu menu = new Menu(mToggle, navigationView, Overview.this);
        //END OF MENU
        //DODATKOWE INFORMACJE
        diagnostic_additional_informations = findViewById(R.id.overview_additional_informations);
        //DODATKOWE INFORMACJE
        textView = findViewById(R.id.overview_tv1date);
        String s = "Wybierz interesującą Cię datę i umów się na kontrolę twojego samochodu pod kątem sprawności technicznej. Mechanik sprawdzi najbardziej istotne elementy twojego auta.\nKoszt usługi: 100-150 PLN ";

        SpannableString ss1 = new SpannableString(s);
        ss1.setSpan(new RelativeSizeSpan(0.92f), 165, 191, 0); // set size
        ss1.setSpan(new ForegroundColorSpan(Color.rgb(255, 200, 70)), 165, 191, 0);// set color
        textView.setText(ss1);
        calendarView = findViewById(R.id.overview_cal);
        ...
        selecthour = findViewById(R.id.overview_btn_hour);
        ...
        //BUTTON CONFIRM
        diagnostic_button_confirm = findViewById(R.id.overview_button_confirm);
        diagnostic_button_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (hour_main.isEmpty() || minute_main.isEmpty()) {
                    Toast.makeText(Overview.this, "Nie wybrano godziny", Toast.LENGTH_SHORT).show();
                    ...
                    Toast.makeText(Overview.this, "Wybrana data pochodzi z przeszłości. Wybierz poprawną datę", Toast.LENGTH_SHORT).show();
                } else if (day_of_week == 1) {
                    Toast.makeText(Overview.this, "Firma nie pracuje w niedziele", Toast.LENGTH_SHORT).show();
                } else if (Integer.parseInt(hour_main) < 8 || Integer.parseInt(hour_main) >= 19 && (day_of_week > 1 && day_of_week < 7)) {
                    Toast.makeText(Overview.this, "Firma w tygodniu pracuje od 8 do 17. Wybierz inną godzinę", Toast.LENGTH_SHORT).show();
                } else if (Integer.parseInt(hour_main) < 8 || Integer.parseInt(hour_main) >= 14 && day_of_week == 7) {
                    Toast.makeText(Overview.this, "Firma w soboty pracuje od 8 do 14. Wybierz inną godzinę", Toast.LENGTH_SHORT).show();
                } else if (Integer.parseInt(day_of_month) == Calendar.getInstance().get(Calendar.DAY_OF_MONTH) &&
                        Integer.parseInt(month_main) == Calendar.getInstance().get(Calendar.MONTH) + 1 &&
                        Integer.parseInt(year_main) == Calendar.getInstance().get(Calendar.YEAR) &&
                    ...
                    Toast.makeText(Overview.this, "Wizyta musi być umówiona co najmniej z około 2-godzinnym wyprzedzeniem", Toast.LENGTH_SHORT).show();
                } else if (choose_mechanic_overview.getText().equals("Wybierz mechanika realizującego usługę")) {
                    Toast.makeText(Overview.this, "Wybierz mechanika", Toast.LENGTH_SHORT).show();
                } else {
                                    ...
                                    if (task.isSuccessful()) {
                                        ...
                                        for (QueryDocumentSnapshot document : task.getResult()) {
                                            ...
                                            if (date.equals(document.get("date"))) {
                                                ...
                                                hour_database = Integer.valueOf(first);
                                                minute_database = Integer.valueOf(second);
                                                ...
                                                hour_activity = Integer.valueOf(first2);
                                                minute_activity = Integer.valueOf(second2);
                                        ...
                                        if (count3 != 0)
                                            Toast.makeText(Overview.this, "Wybrany mechanik w tym dniu w tych godzinach jest zajęty. Spróbuj wybrać inną godzinę. ", Toast.LENGTH_SHORT).show();
                                        if (count3 == 0) Add();
                                    } else {
                                    }
                                }
    ...
    private void Add() {
        //CHECK IF MECHANIC IS FREE
        DocumentReference documentReference = fstore.collection("overview").document();
        ...
        if (user != null) {
            ...
            history_to_database.put("name_of_service", "Przegląd samochodu");
        ...
        documentReference.set(diagnostic_to_database).addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                ...
                Toast.makeText(Overview.this, "Pomyślnie umówiono się na wizytę", Toast.LENGTH_SHORT).show();
            }
        });
        //MECHANIC SCHEDULE
        ...
        mechanics_schedule.put("service", "Przegląd samochodu");
        mechanics_schedule.put("repair_price", "100-150");
        if (user == null) {
    ...
    private void setUpRecyclerView() {
        ...
        RecyclerView recyclerView = myDialog_overview.findViewById(R.id.recycler_view);
    ...
    public void ShowPopUp(View v) {
        myDialog_overview.setContentView(R.layout.custompopup);
        Window window = myDialog_overview.getWindow();
        WindowManager.LayoutParams wlp = window.getAttributes();
        wlp.gravity = Gravity.TOP;
        wlp.y = 400;
        wlp.flags &= ~WindowManager.LayoutParams.FLAG_DIM_BEHIND;
        window.setAttributes(wlp);
        myDialog_overview.getWindow().setBackgroundDrawable(getResources().getDrawable(R.drawable.rounded_background));
        TextView close = myDialog_overview.findViewById(R.id.close_popup);
    ...
    public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
        TextView textView = (TextView) findViewById(R.id.overview_hour);
